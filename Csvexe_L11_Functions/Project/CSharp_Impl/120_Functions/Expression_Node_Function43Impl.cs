...
namespace Xenon.Functions
{
    ...
    public class Expression_Node_Function43Impl : Expression_Node_FunctionAbstract
    {
        ...
        {
            Expression_Node_Function f0 = new Expression_Node_Function43Impl(this.EnumEventhandler,this.ListS_ArgName,this.Functiontranslatoritem);
        ...
        public override string Expression_ExecuteMain(Log_Reports pg_Logging)
        {
            ...
            if (this.EnumEventhandler == EnumEventhandler.O_Wr)
            {
                ...
                this.Perform2(this.ExpressionfncPrmset.Sender, pg_Logging);
            ...
            )
        {
            ...
            if (pg_Logging.BSuccessful)
            {
                ...
                this.TrySelectAttr(out ec_ArgNameVariable, Expression_Node_Function43Impl.S_PM_NAME_VAR, false, Request_SelectingImpl.Unconstraint, pg_Logging);
                string sVariableName = ec_ArgNameVariable.Execute_OnExpressionString(Request_SelectingImpl.Unconstraint, pg_Logging);

                Expression_Node_String ec_ArgFcName;
                this.TrySelectAttr(out ec_ArgFcName, Expression_Node_Function43Impl.S_PM_NAME_FC, false, Request_SelectingImpl.Unconstraint, pg_Logging);
                List<Usercontrol> list_UcFc = this.Owner_MemoryApplication.MemoryForms.GetUsercontrolsByName(ec_ArgFcName, true, pg_Logging);
                foreach (Usercontrol uct in list_UcFc)
                {
                    if (uct is UsercontrolCheckbox)
                    {
                        ...
                        XenonName o_VariableName = new XenonNameImpl(sVariableName, this.Cur_Givechapterandverse);
                    ...
                    {
                        // エラー
                        err_SFcName = uct.ControlCommon.Expression_Name_Control.Execute_OnExpressionString(Request_SelectingImpl.Unconstraint, pg_Logging);
